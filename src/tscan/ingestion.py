...
from tscan.types import DocumentPage, LoadedDocument
...
def _resize_if_needed(image: Image.Image, max_megapixels: int) -> Image.Image:
    ...
    resized.info["tscan_resized_from"] = [image.width, image.height]
    return resized
...
def load_document(path: str | Path, settings: Settings) -> LoadedDocument:
    ...
    preprocessing_log: list[dict[str, object]] = [
        {"action": "signature_check", "media_type": media_type, "byte_size": byte_size}
    ]
    if media_type == "application/pdf":
        pages = _load_pdf(source, settings)
        preprocessing_log.append(
            {"action": "pdf_render", "dpi": settings.pdf_dpi, "page_count": len(pages)}
        )
    elif source.suffix.lower() in IMAGE_SUFFIXES or media_type.startswith("image/"):
        pages = _load_image(source, settings)
        preprocessing_log.append({"action": "image_decode_and_exif_transpose"})
    else:
        raise DocumentError(f"Unsupported media type: {media_type}")

    preprocessing_log.append(
        {
            "action": "page_dimensions",
            "pages": [
                {
                    "page_number": page.page_number,
                    "width": page.width,
                    "height": page.height,
                    "resized": "tscan_resized_from" in page.image.info,
                    "resized_from": page.image.info.get("tscan_resized_from"),
                }
                for page in pages
            ],
