...
from tscan.exceptions import ModelError, TScanError
...
LOGGER = logging.getLogger("tscan.api")
...
async def _save_upload(upload: UploadFile, settings: Settings) -> Path:
    ...
    try:
        with tempfile.NamedTemporaryFile(
            prefix="tscan-", suffix=_safe_suffix(upload.filename), delete=False
        ) as handle:
            path = Path(handle.name)
            async def read_chunk() -> bytes:
                if settings.model_provider == "mock":
                    return upload.file.read(1024 * 1024)
...
async def _extract_document(
    ...
    **kwargs: object,
    ...
    # Mock inference is deterministic and CPU-trivial. Keeping it inline also makes
    # smoke tests independent from platform-specific thread-executor behavior.
    if settings.model_provider == "mock":
        return pipeline.extract(source, **kwargs)
    return await asyncio.to_thread(pipeline.extract, source, **kwargs)
...
def create_app(
    ...
    @app.exception_handler(TScanError)
    async def handle_tscan_error(_request: Request, exc: TScanError) -> JSONResponse:
    ...
    async def health() -> dict[str, object]:
        return {
            ...
            "model": settings.model_name if settings.model_provider != "mock" else "tscan-mock",
        }
    ...
    async def batch(
        ...
        try:
            ...
            with tempfile.NamedTemporaryFile(
                prefix="tscan-batch-", suffix=".zip", delete=False
            ) as archive_handle:
            ...
            return FileResponse(
                ...
                filename="tscan-batch-results.zip",
