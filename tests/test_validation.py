...
def test_keeps_ambiguous_locale_amount_raw() -> None:
    schema = {
        "type": "object",
        "properties": {"total": {"type": "number"}},
        "required": ["total"],
    }
    result = validate_and_normalize('{"total":"1.234"}', schema, language="auto")
    assert result.parsed == {"total": "1.234"}
    assert result.normalized == {"total": "1.234"}
    assert result.schema_valid is False
    assert any(issue.code == "locale_normalization_ambiguous" for issue in result.issues)
